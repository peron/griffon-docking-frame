...
// check to see if we already have a DockingFrameGriffonAddon
...
slurpedBuilder1.each() { prefix, v ->
    v.each { builder, views ->
        addonIsSet1 = addonIsSet1 || 'DockingFrameGriffonAddon' == builder
    }
}

if (addonIsSet1) {
    println 'Removing DockingFrameGriffonAddon from Builder.groovy'
    def builderConfigFile1 = new File("${basedir}/griffon-app/conf/Builder.groovy")
    builderConfigFile1.text = builderConfigFile1.text - "root.'DockingFrameGriffonAddon'.addon=true\n"
}
